document.addEventListener('DOMContentLoaded', function() {
    ...
    function nextLevel() {
        level++;
        boardSize = Math.min(8, 5 + Math.floor(level / 3));
        initGame();
    }
    ...
    function generateColors(size) {
        const colorCount = Math.min(8, Math.max(3, Math.floor(size * 0.8)));
    ...
    function handleCellClick(x, y) {
        ...
        if (cellValue > 0) {
            // If we were already drawing a path for this color, complete it
